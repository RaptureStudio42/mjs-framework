...
30. [🧩 Modules cœur & personnalisation](30-modules-coeur.md) — bibliothèque prête à l'emploi (`<@select>`, `<@field>`, `<@checkbox>`, `<@radio>`, `<@switch>`, `<@color>`) et l'info-bulle universelle `@title` ; doctrine de personnalisation en trois leviers (variables `--mjs-*`, `::part()`, éjection du source sous le même nom) sous une règle commune de fair-play CSS (`:where()`)
