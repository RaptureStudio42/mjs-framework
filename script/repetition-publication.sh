...
TARBALL="$(ls "$PACK_DIR"/mjs-framework-*.tgz 2> /dev/null | head -n1)"
if [ -z "$TARBALL" ]; then
...
step "import('mjs-framework') résout" node -e "import('mjs-framework').then(() => process.exit(0)).catch(e => { console.error(e); process.exit(1) })"
step "import('mjs-framework/mjs-server') résout" node -e "import('mjs-framework/mjs-server').then(() => process.exit(0)).catch(e => { console.error(e); process.exit(1) })"
