...
La page affiche, en tête, le nombre de variables du projet, puis un champ de recherche et un filtre par **nature** de la déclaration (`module`, `theme`, `framework`, `stylesheet`), puis la liste. Chaque ligne porte le nom de la variable, une pastille de sa couleur quand la valeur en est une (hexadécimal, `rgb`, `hsl`, `oklch`), une étiquette de nature, qui la déclare, et combien d'endroits la lisent. Déplier une ligne montre chaque déclaration avec son fichier et son numéro de ligne, le commentaire de documentation quand il y en a un, et la liste complète des lecteurs. Une variable déclarée à plusieurs endroits est signalée comme telle, avec un rappel que c'est le plus proche qui gagne.
